...
def kirim_perintah_dfplayer(perintah, param1=0x00, param2=0x00):
    """Mengirim paket perintah 10 byte standar dengan Checksum ke DFPlayer Mini"""
    if not DFPLAYER_AVAILABLE or df_serial is None: 
        return
    
    # Hitung Checksum standar DFPlayer Mini
    versi = 0xFF
    panjang = 0x06
    feedback = 0x00  # 0x00 = Tanpa feedback, 0x01 = Dengan feedback
...
if __name__ == "__main__":
    # 1. Inisialisasi awal aplikasi Tkinter
    app = App()
    
    # 2. Bunyi BEEP 3 kali saat pertama kali sistem sukses dinyalakan
    bunyi_buzzer_sync(3)
    
    # 3. Jalankan loop utama grafis
    app.mainloop()

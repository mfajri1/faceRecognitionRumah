"""
SISTEM KEAMANAN PINTU: WAJAH (MEDIAPIPE) + SUARA (EDISI ULTRA-SMOOTH RASPI 3B+)
===================================================================
...
# --- INTEGRASI MEDIAPIPE FACE DETECTION (RINGAN & TAHAN CAHAYA) ---
...
MIN_SAMPLES = 9 
FACE_SIZE = (200, 200) 
LBPH_THRESHOLD = 60  # Toleransi LBPH dinaikkan untuk fleksibilitas antar-ruangan
...
# ============================================================
# UTILITAS HARDWARE, DETEKSI WAJAH & MIKROFON
# ============================================================

def normalisasi_cahaya(gray_crop):
    """Meratakan kontras/cahaya wajah menggunakan CLAHE agar akurat di semua ruangan."""
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    return clahe.apply(gray_crop)

def dapatkan_wajah_terbesar(frame_bgr):
    """
    Deteksi Wajah MediaPipe dengan Optimasi Downscaling (320x240)
    Sangat ringan untuk Raspberry Pi 3B+ agar tampilan kamera lancar tanpa lag.
    """
    if frame_bgr is None:
        return None

    h_orig, w_orig = frame_bgr.shape[:2]
    # Downscale gambar khusus untuk proses deteksi AI yang cepat
    ...
    for detection in results.detections:
        ...
        # Kembalikan koordinat bounding box ke resolusi frame asli
        x = int(bbox.xmin * w_orig)
        y = int(bbox.ymin * h_orig)
        w = int(bbox.width * w_orig)
        h = int(bbox.height * h_orig)
        
        # Batasi agar koordinat tidak merusak batas gambar
        x = max(0, x)
        y = max(0, y)
        w = min(w, w_orig - x)
        h = min(h, h_orig - y)
    ...
    # Ambil wajah dengan ukuran terbesar (terdekat ke kamera)
...
def train_model():
    ...
    for nama_user in sorted(os.listdir(DATASET_DIR)):
        ...
        for nama_file in file_foto:
            ...
            img = normalisasi_cahaya(img)  # Normalisasi cahaya dataset
            faces.append(img)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Sistem Keamanan Pintu: Wajah (MediaPipe) + Suara")
    ...
    # ============================================================
    # 1. HALAMAN UTAMA (SISTEM UTAMA WAJAH MEDIAPIPE)
    # ============================================================
    ...
    def update_su_camera(self):
        ...
        if ret:
            ...
            # Deteksi Wajah Cepat dengan MediaPipe
    ...
    def alur_keamanan_sekuensial(self):
        ...
        # Proses Crop & Normalisasi Cahaya Wajah
        ...
        if wajah is not None:
            ...
            if face_crop.size > 0:
                ...
                face_img = normalisasi_cahaya(face_img)  # Normalisasi cahaya
    ...
    def dw_ambil_foto(self):
        ...
        face_img = cv2.resize(face_crop, FACE_SIZE)
        face_img = normalisasi_cahaya(face_img)  # Normalisasi cahaya sebelum disimpan

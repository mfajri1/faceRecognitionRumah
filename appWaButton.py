"""
SISTEM KEAMANAN PINTU: WAJAH (OPENCV DNN) + TOMBOL AKSES (GPIO26) & SERVO DOOR
===================================================================
...
3. Mode Akses dapat dipilih di GUI: WEBCAM (deteksi wajah otomatis) atau TOMBOL (tombol fisik di GPIO26)
...
BUTTON_PIN = 26               # Tombol Akses Masuk (aktif LOW, tombol ke GND, pull-up internal)
...
if GPIO_AVAILABLE:
    ...
    # Tombol akses masuk (GPIO26) - tombol ke GND, pull-up internal, aktif LOW saat ditekan
...
def tombol_ditekan():
    """True jika tombol akses (GPIO26) sedang ditekan (aktif LOW)."""
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.access_mode = tk.StringVar(value="webcam")  # "webcam" atau "button"

        reset_semua_komponen_standby()
        self.build_sistem_utama()

        # Thread pemantau tombol akses (GPIO26), berjalan terus selama aplikasi hidup
    ...
    # ============================================================
    # PEMANTAU TOMBOL AKSES (GPIO26)
    # ============================================================
    def monitor_button(self):
        """Berjalan di background. Saat mode akses = 'button' dan tombol ditekan,
        pintu langsung dibuka tanpa perlu verifikasi wajah."""
        while self.button_monitor_running:
            try:
                if (self.access_mode.get() == "button"
                        and tombol_ditekan()
    ...
    def alur_akses_tombol(self):
        try:
            ...
            lcd_cetak("=== AKSES TOMBOL ===", "Tombol Ditekan", "SILAHKAN MASUK", "PINTU TERBUKA")
    ...
    def build_sistem_utama(self):
        ...
        mode_frame = tk.Frame(top_bar, bg=COLOR_BG)
        mode_frame.pack(side="left", padx=15, pady=10)
        tk.Label(mode_frame, text="Mode Akses:", font=("Segoe UI", 9, "bold"), bg=COLOR_BG, fg=COLOR_TEXT).pack(side="left", padx=(0, 8))
        tk.Radiobutton(mode_frame, text="Webcam", font=("Segoe UI", 9), bg=COLOR_BG, fg=COLOR_TEXT,
                        selectcolor=COLOR_BG, activebackground=COLOR_BG, activeforeground=COLOR_TEXT,
                        variable=self.access_mode, value="webcam", command=self.on_mode_change).pack(side="left", padx=5)
        tk.Radiobutton(mode_frame, text="Tombol", font=("Segoe UI", 9), bg=COLOR_BG, fg=COLOR_TEXT,
                        selectcolor=COLOR_BG, activebackground=COLOR_BG, activeforeground=COLOR_TEXT,
                        variable=self.access_mode, value="button", command=self.on_mode_change).pack(side="left", padx=5)
        ...
        if self.su_recognizer is None:
            ...
            self.su_status_var.set("Mode Webcam aktif. Berdiri di depan kamera untuk verifikasi wajah...")
            reset_semua_komponen_standby()
    ...
    def on_mode_change(self):
        ...
        if mode == "webcam":
            self.su_status_var.set("Mode Webcam aktif. Berdiri di depan kamera untuk verifikasi wajah...")
            lcd_cetak("=== MODE WEBCAM ===", "Deteksi Wajah Aktif", "Silahkan Berdiri", "Di Depan Kamera")
        else:
            self.su_status_var.set("Mode Tombol aktif. Tekan tombol akses untuk membuka pintu.")
            lcd_cetak("=== MODE TOMBOL ===", "Deteksi Wajah OFF", "Tekan Tombol Akses", "Untuk Buka Pintu")
    ...
    def update_su_camera(self):
        ...
        if ret:
            ...
            if self.access_mode.get() == "webcam":
                ...
                cv2.putText(display, "MODE TOMBOL AKTIF", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)

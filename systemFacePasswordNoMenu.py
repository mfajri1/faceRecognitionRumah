...
import requests  # ◄ Tambahkan baris ini agar library requests dikenali!
...
class App(tk.Tk):
    ...
    def update_su_camera(self):
        ...
        if ret:
            ...
            # Deteksi & pengenalan wajah otomatis, tanpa perlu klik tombol apapun.
            if (
                not self.su_processing
                and self.su_recognizer is not None
                and len(faces) > 0
            ):
                self.su_try_recognize(gray, faces[0])
    ...
    # ------------------------------------------------------------------
    # UTILITY: KIRIM NOTIFIKASI WHATSAPP
    # ------------------------------------------------------------------
    def kirim_notif_wa(self, nama_user):
        """Mengirim notifikasi ke WA menggunakan API Gateway (Contoh: Fonnte/GreenAPI)"""
        # Ganti dengan URL, Token, dan Nomor WA kamu sendiri
        url = "https://api.fonnte.com/send" 
        token = "RgMZQCTAy8DGLNEMqphk"
        nomor_tujuan = "628136554516" # Gunakan kode negara, misal: 628123456789

        pesan = (
            f"⚠️ *PERINGATAN KEAMANAN* ⚠️\n\n"
            f"Ada upaya masuk ke sistem!\n"
            f"👤 *Wajah Terdeteksi:* {nama_user}\n"
            f"❌ *Status:* ANDA SALAH MEMASUKKAN PASSWORD SUARA.\n\n"
            f"Mohon periksa sistem jika ini bukan Anda."
        )
        ...
        try:
            # Menggunakan timeout agar aplikasi tidak hang jika jaringan lambat
            response = requests.post(url, data=payload, headers=headers, timeout=5)
            print("Respon WA:", response.text)
        except Exception as e:
            print(f"Gagal mengirim WhatsApp: {e}")

    # ------------------------------------------------------------------
    # MODIFIKASI PADA METHOD PROSES PASSWORD
    # ------------------------------------------------------------------
    def su_proses_password(self):
        spoken_text = speech_to_text(status_callback=self.su_set_status_threadsafe)
        ...
        if stored_hash is not None and stored_hash == input_hash:
            ...
            # 🟢 PANGGIL FUNGSI KIRIM WA DI SINI (Berjalan di background thread)
